...
export function Project({ ProjectDesign, ProjectDoneDate, ProjectName, ProjectType, ProjectRepository }: ProjectProps) {
  ...
  return (
    <div className="flex w-[650px] gap-2 items-center justify-center max-md:justify-start p-4 my-1 rounded-lg text-base hover:bg-neutral-300/70 dark:hover:bg-neutral-800/70 mx-auto max-lg:w-full max-md:overflow-y-auto max-md:border-b-2 max-sm:text-xs">
      <span className="font-semibold max-sm:w-[80px] max-sm:justify-start whitespace-nowrap truncate text-overflow-ellipsis max-2sm:w-[90px]">{ProjectName}</span>
     
      <Separator className="bg-neutral-700 dark:bg-neutral-300 w-40 max-2sm:h-5 max-2sm:w-px max-md:w-12 max-md:h-px" />
     
      <span className="whitespace-nowrap truncate text-overflow-ellipsis max-sm:w-[80px] max-2sm:w-[90px] " >{ProjectType}</span>
